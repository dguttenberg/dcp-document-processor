...
exports.seedFromExtraction = seedFromExtraction;
exports.markDocumentError = markDocumentError;
const supabase_1 = require("./supabase");
// ── Seed opportunity fields (empty-only writes) ──────────────────────
async function seedOpportunity(opportunityId, opp) {
    const populated = [];
    const skipped = [];
    // Read current state
    const { data: rawCurrent, error } = await (0, supabase_1.getSupabase)()
        .from('opportunities')
    ...
    if (Object.keys(update).length > 0) {
        const { error: updateError } = await (0, supabase_1.getSupabase)()
            .from('opportunities')
            .update(update)
            .eq('id', opportunityId);
        if (updateError) {
...
async function seedRfiQuestions(opportunityId, questions) {
    if (!questions || questions.length === 0)
        return 0;
    // Get existing questions to find max sort_order and avoid exact dupes
    const { data: rawExisting } = await (0, supabase_1.getSupabase)()
        .from('rfi_questions')
...
async function seedContacts(opportunityId, contacts) {
    if (!contacts || contacts.length === 0)
        return 0;
    // Get existing contacts to avoid dupes
    const { data: rawExistingContacts } = await (0, supabase_1.getSupabase)()
        .from('client_contacts')
        .select('full_name')
        .eq('opportunity_id', opportunityId);
    const existingNames = new Set((rawExistingContacts ?? []).map(c => c.full_name.toLowerCase().trim()));
...
async function seedAssessmentScores(opportunityId, signals) {
    if (!signals || signals.length === 0)
        return [];
    const populated = [];
    // Get the AI-prefill criteria from the database
    const { data: rawCriteria } = await (0, supabase_1.getSupabase)()
    ...
    const { data: rawResponses } = await (0, supabase_1.getSupabase)()
        .from('assessment_responses')
        .select('criterion_id, is_confirmed')
        .eq('opportunity_id', opportunityId);
    const existingResponses = (rawResponses ?? []);
    const confirmedIds = new Set(existingResponses.filter(r => r.is_confirmed).map(r => r.criterion_id));
    const existingIds = new Set(existingResponses.map(r => r.criterion_id));
    for (const signal of signals) {
        ...
        if (confirmedIds.has(criterion.id))
            continue;
...
async function writeExtractedIntel(documentId, extraction, status) {
    const { error } = await (0, supabase_1.getSupabase)()
        .from('documents')
        .update({
        ingestion_status: status,
        extracted_intel: extraction,
    })
        .eq('id', documentId);
    if (error) {
...
async function seedFromExtraction(opportunityId, documentId, extraction) {
...
async function markDocumentError(documentId, errorMsg) {
    await (0, supabase_1.getSupabase)()
        .from('documents')
        .update({
